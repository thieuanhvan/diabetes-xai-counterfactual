...
maintainer revisiting this repository for a follow-up study or extension.
Not customer-facing.
...
BRFSS 2021 CSV
    ...
    ->  DiCE CF generation in two modes  (src/pipelines/counterfactual/)
            global:    single features_to_vary list, immutables excluded
...
colsample_bytree=0.9, tree_method='hist'). Hyperparameter values are
documented in `configs/default.yaml` and frozen by `seed_everything(42)` for
reproducibility.

`src/pipelines/counterfactual/feature_taxonomy.py`: the 5-class taxonomy
lives here. Each feature has one `Mutability` enum value
...
hardware, library versions, seeds and git-commit.
...
bar chart). Title text inside figures is English by convention; caption
numbering lives in the manuscript, never in the image.
...
Rationale: BRFSS 2021 is the latest BRFSS release providing the diabetes-binary
schema in the julnazz/Teboul Kaggle convention used widely in the diabetes
ML literature. The 236,378-record size supports both training and a 47,276-patient
held-out test set without resampling. NHIS and NHANES were considered but
deferred as candidate datasets for a future cross-dataset extension.

Trade-off accepted: self-reported BMI in BRFSS limits clinical fidelity
versus NHANES measured BMI. Documented in the manuscript's external-validity
discussion.

### ADR-2: classifier = single XGBoost

Considered alternatives: ensemble of LR + RF + XGBoost, single LR
(interpretable baseline), single neural net.

Decision: single XGBoost.

Rationale: the methodological contribution of this paper is the taxonomy and
its per-query application, not the underlying classifier. Holding the
classifier fixed across modes isolates the experimental effect of the
taxonomy and avoids confounding the comparison with classifier-architecture
differences. Single model also reduces compute (one CF generation pass per
mode versus three across an ensemble).

Trade-off accepted: results are model-specific. Generalization to LR or RF
is left to future work; method comparison (random / genetic / kdtree) is
ablated in the method ablation grid, but cross-classifier generalization
remains open.
...
Method ablation confirmed the trade-off empirically. Genetic improves
validity to ~0.993 and actionability to ~1.000 but doubles sparsity (0.139
vs 0.072) and runs at ~51 minutes; it remains a high-quality alternative
when compute budget is not constrained. kdtree failed structurally - 0/200
counterfactuals were found in per-query mode because nearest training
neighbours of high-risk queries violate the taxonomy permitted_range on >=8/21
features simultaneously. A dtype incompatibility between DiCE 0.12 and
pandas 3.x (float32 downcast in `PublicData._set_feature_dtypes` vs float64
scalar assignment in posthoc_sparsity) compounded the issue; the 3-line
workaround in `dice_runner.py` unblocks the run but does not address the
structural failure.
...
Decision: per-query application. For each patient x_i, derive features_to_vary
and permitted_range from x_i baseline values.
...
allow [0, 1]) would let DiCE suggest reducing PhysActivity for a patient
who already has it, even though such a recommendation is non-actionable.
...
(0.752 -> 0.808) and eliminates wrong-direction violations entirely.
...
The class-balance ablation tested thr in {0.0, 0.5, 0.7} and confirmed the
taxonomy operates independently of cohort risk profile; wrong-direction
violations remained 0 across all three thresholds.

### ADR-7: repo layout = src/pipelines + src/utils + analysis

Considered alternatives: flat `src/` with all modules at one level, nested
...
Pipeline steps are core code with unit tests and are run by `run_main.py`
once per experiment. Analysis scripts are derived computations on pipeline
outputs; they can be re-run any time on an existing run without re-training
and they write back to outputs/. Separating them prevents accidentally
treating analysis as a pipeline step. src/utils/ stays at src level (not
under pipelines/) because logger and seed are shared by both pipelines and
analysis.
...
### ADR-8: reproducibility = log + JSON sidecar + hardware capture

Considered alternatives: log file only, requirements.txt pinning only, MLflow
tracking, no reproducibility artifacts.

Decision: per-run log + per-run JSON sidecar capturing library versions,
seeds, hardware (CPU, RAM, OS) and git commit. Both files live in `outputs/`.
...
Trade-off accepted: extra ~5 KB per run for the JSON. Negligible.
