...
inputs are NOT overwritten. The next two entries are REAL rows from the
BRFSS 2021 test set, identified by their rank in the top-200 high-risk
cohort; they exist so a demo can be run on an actual patient from the
population the Action phase targets, rather than on a hand-tuned profile.
All remaining entries are CURATED archetypes, which trade real-patient
authenticity for clinical coherence (each tells one clear story). Every
non-sentinel value is a full 21-feature dict suitable for direct
assignment to st.session_state["input_<feature>"].
...
PRESETS: Dict[str, Optional[Dict[str, float]]] = {
    ...
    # ────────────────────────────────────────────────────────────────
    # Real patients lifted verbatim from the BRFSS 2021 test set, by rank
    # in the top-200 high-risk cohort. Unlike the curated archetypes below,
    # these are actual rows — nothing was hand-tuned to make the risk look
    # high. Values verified against demo/models/X_test.parquet and the
    # predicted probabilities in demo/models/proba_test.parquet.
    # ────────────────────────────────────────────────────────────────
    "Top-200 rank 1 — real BRFSS 2021 test patient (P=0.887)": {
        "Age": 7, "Sex": 1, "Education": 4, "Income": 5,
        "BMI": 56.0,
        "HighBP": 1, "HighChol": 1, "Stroke": 0, "HeartDiseaseorAttack": 1, "DiffWalk": 1,
        "Smoker": 1, "PhysActivity": 0, "Fruits": 1, "Veggies": 1, "HvyAlcoholConsump": 0,
        "AnyHealthcare": 1, "NoDocbcCost": 0, "CholCheck": 1,
        "GenHlth": 5, "MentHlth": 15, "PhysHlth": 30,
    },

    "Top-200 rank 32 — real BRFSS 2021 test patient (P=0.786)": {
        "Age": 6, "Sex": 1, "Education": 5, "Income": 5,
        "BMI": 45.0,
        "HighBP": 1, "HighChol": 1, "Stroke": 1, "HeartDiseaseorAttack": 0, "DiffWalk": 0,
        "Smoker": 0, "PhysActivity": 0, "Fruits": 0, "Veggies": 0, "HvyAlcoholConsump": 0,
        "AnyHealthcare": 1, "NoDocbcCost": 0, "CholCheck": 1,
        "GenHlth": 5, "MentHlth": 0, "PhysHlth": 0,
    },
